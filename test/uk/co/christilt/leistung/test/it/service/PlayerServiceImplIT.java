...
/**
 * Integration tests for the {@link PlayerService} class with Spring
 * configuration.
 */
@ActiveProfiles("dev")
...
public class PlayerServiceImplIT extends AbstractServiceIT<Player, Long> {

	private static Logger LOGGER = Logger.getLogger(PlayerServiceImplIT.class);
	...
	public void setService(Service<Player, Long> service) {
	...
	/*
	 * Test additional methods present in PlayerService.
	 */
	...
	public void RetrieveByName_NullAsParameter_NullPointerExceptionThrown() {
		((PlayerService) service).retrieveByName(null);
		fail();
	}
	...
	public void RetrieveByName_NoEntityWithMatchingNameIsPresent_NoSuchElementExceptionThrown() {
		jdbcTemplate.update(this.insertEntityOneSqlString);
		((PlayerService) service).retrieveByName("Jonny");
		fail();
	}
	...
	public void RetrieveByName_EntityWithMatchingNameIsPresent_EntityRetrievedWithMatchingName() {
		...
		jdbcTemplate.update("INSERT INTO player (name, squad_no, position)"+
				"VALUES ('"+name+"', 1, 'gk')");
		Player retrieved = ((PlayerService) service).retrieveByName(name);
	...
	public void RetrieveBySquadNo_ZeroAsParameter_IllegalArgumentExceptionThrown() {
		((PlayerService) service).retrieveBySquadNo(0);
		fail();
	}
	...
	public void RetrieveBySquadNo_NoEntityWithMatchingNameIsPresent_NoSuchElementExceptionThrown() {
		jdbcTemplate.update(this.insertEntityOneSqlString);
		((PlayerService) service).retrieveBySquadNo(100);
		fail();
	}
	...
	public void RetrieveBySquadNo_EntityWithMatchingNameIsPresent_EntityRetrievedWithMatchingSquadNo() {
		...
		Player retrieved = ((PlayerService) service).retrieveBySquadNo(squadNo);
